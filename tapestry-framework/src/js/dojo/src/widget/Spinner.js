...
dojo.provide("dojo.widget.Spinner");dojo.require("dojo.html.*");dojo.require("dojo.html.layout");dojo.require("dojo.string");dojo.require("dojo.widget.*");dojo.require("dojo.widget.IntegerTextbox");dojo.require("dojo.widget.RealNumberTextbox");dojo.require("dojo.widget.DateTextbox");dojo.require("dojo.experimental");dojo.declare(
"dojo.widget.Spinner",null,{_typamaticTimer: null,_typamaticFunction: null,_currentTimeout: this.defaultTimeout,_eventCount: 0,defaultTimeout: 500,timeoutChangeRate: 0.90,templatePath: dojo.uri.moduleUri("dojo.widget", "templates/Spinner.html"),templateCssPath: dojo.uri.moduleUri("dojo.widget", "templates/Spinner.css"),inputWidget: null,_handleKeyEvents: function( evt){if(!evt.key){ return; }
...
this._eventCount++;},_onSpinnerKeyUp: function( evt){this._arrowReleased(evt);this.onkeyup(evt);},_pressButton: function( node){dojo.html.addClass(node, "dojoSpinnerButtonPushed");},_releaseButton: function( node){dojo.html.removeClass(node, "dojoSpinnerButtonPushed");},_arrowPressed: function( evt,  direction){var nodePressed = (direction == -1) ? this.downArrowNode : this.upArrowNode;var nodeReleased = (direction == +1) ? this.downArrowNode : this.upArrowNode;if(typeof evt != "number"){if(this._typamaticTimer != null){if(this._typamaticNode == nodePressed){return;}
...
if(typeof this.textbox.selectionEnd == "number"){this.textbox.selectionEnd = x;}else if(this.textbox.createTextRange){var range = this.textbox.createTextRange();range.collapse(true);range.moveEnd('character', x);range.moveStart('character', x);range.select();}}catch(e){  }},fillInTemplate: function( args,  frag){if (this.inputWidget){this.inputWidget.prototype.fillInTemplate.call(this, args, frag);}
var source = this.getFragNodeRef(frag);dojo.html.copyStyle(this.domNode, source);if(this.textbox.addEventListener){this.textbox.addEventListener('DOMMouseScroll', dojo.lang.hitch(this, "_mouseWheeled"), false);}else{dojo.event.connect(this.textbox, "onmousewheel", this, "_mouseWheeled");}},onValueChanged: function(value){},setValue: function(value){if (this._oldValue != value) {this._oldValue = value;if (this.inputWidget){this.inputWidget.prototype.setValue.call(this, value);}
this.onValueChanged(value);}}
}
);dojo.widget.defineWidget(
"dojo.widget.IntegerSpinner",[dojo.widget.IntegerTextbox, dojo.widget.Spinner],{delta: "1",inputWidget: dojo.widget.IntegerTextbox,adjustValue: function( direction,  x){var val = this.getValue().replace(/[^\-+\d]/g, "");if(val.length == 0){ return; }
var num = Math.min(Math.max((parseInt(val)+(parseInt(this.delta) * direction)), ((this.flags.min||this.flags.min==0)?this.flags.min:-Infinity)), ((this.flags.max||this.flags.max==0)?this.flags.max:+Infinity));val = num.toString();if(num >= 0){val = ((this.flags.signed == true)?'+':' ')+val;}
...
"dojo.widget.RealNumberSpinner",[dojo.widget.RealNumberTextbox, dojo.widget.Spinner],function(){ dojo.experimental("dojo.widget.RealNumberSpinner"); },{delta: "1e1",inputWidget: dojo.widget.RealNumberTextbox,adjustValue: function( direction,  x){var val = this.getValue().replace(/[^\-+\.eE\d]/g, "");if(!val.length){ return; }
...
"dojo.widget.TimeSpinner",[dojo.widget.TimeTextbox, dojo.widget.Spinner],function(){ dojo.experimental("dojo.widget.TimeSpinner"); },{inputWidget: dojo.widget.TimeTextbox,adjustValue: function( direction,  x){var val = this.getValue();var format = this.flags.format = ((typeof this.displayFormat == "string") && this.displayFormat.length > 0) ? this.displayFormat : "hh:mm:ss t";if(direction == 0 || !val.length || !this.isValid()){ return; }
