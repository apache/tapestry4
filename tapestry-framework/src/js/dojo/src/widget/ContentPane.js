...
"dojo.widget.ContentPane",dojo.widget.HtmlWidget,function(){this._styleNodes =  [];this._onLoadStack = [];this._onUnloadStack = [];this._callOnUnload = false;this._ioBindObj;this.scriptScope;this.bindArgs = {};}, {isContainer: false,adjustPaths: true,href: "",extractContent: true,parseContent:true,cacheContent:true,preload: false,refreshOnShow: false,handler: "",executeScripts: false,scriptSeparation: true,loadingMessage: "Loading...",isLoaded: false,"class": "dojoContentPane",templateCssPath: dojo.uri.moduleUri("dojo", "widget/templates/ContentPane.css"),postCreate: function(args, frag, parentComp){dojo.html.addClass(this.domNode, this["class"]);var source = this.getFragNodeRef(frag);if(source != this.domNode){dojo.html.moveChildren(source, this.containerNode || this.domNode);}
if (this.handler!==""){this.setHandler(this.handler);}
if(this.isShowing() || this.preload){this._prepareForShow();}},show: function(){if(this.refreshOnShow){this.refresh();}else{this._prepareForShow();}
dojo.widget.ContentPane.superclass.show.call(this);},refresh: function(){this.isLoaded=false;this._prepareForShow();},_prepareForShow: function() {if ( this.isLoaded ){return;}
if ( dojo.lang.isFunction(this.handler)) {this._runHandler();} else if ( this.href != "" ) {this._downloadExternalContent(this.href, this.cacheContent && !this.refreshOnShow);} else {if(this.parseContent){this._createSubWidgets();}
this.isLoaded=true;}},setUrl: function( url) {this.href = url;this.isLoaded = false;if ( this.preload || this.isShowing() ){this._prepareForShow();}},abort: function(){var bind = this._ioBindObj;if(!bind || !bind.abort){ return; }
...
if(_self.parseContent){_self._createSubWidgets();}
_self.onResized();_self.onLoad();}
if(dojo.hostenv.isXDomain && data.requires.length){dojo.addOnLoad(asyncParse);}else{asyncParse();}}
},_createSubWidgets: function(){var node = this.containerNode || this.domNode;var parser = new dojo.xml.Parse();var frag = parser.parseElement(node, null, true);dojo.widget.getParser().createSubComponents(frag, this);},setHandler: function( handler) {var fcn = dojo.lang.isFunction(handler) ? handler : window[handler];if(!dojo.lang.isFunction(fcn)) {this._handleDefaults("Unable to set handler, '" + handler + "' not a function.", "onExecError", true);return;}
