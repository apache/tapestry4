...
myObj=myObj[p];}
return myObj;},_setToContainers: function (obj,widget) {for(var i=0, len=widget.children.length; i<len; ++i) {var currentWidget=widget.children[i];if (currentWidget == null) { continue; }
if (currentWidget.widgetType == "Repeater") {for(var j=0,len=currentWidget.getRowCount(); j<len; ++j) {var foo=currentWidget._initRow(j);}}
this._setToContainers (obj, currentWidget);switch(currentWidget.widgetType) {case "Checkbox":
currentWidget.setValue(currentWidget.inputNode.checked);break;case "DropdownDatePicker":
if (currentWidget.getValue() != '') {currentWidget.setValue(currentWidget.getValue());}
break;case "Select":
continue;break;case "ComboBox":
break;case "CurrencyTextbox":
case "DateTextbox":
case "IntegerTextbox":
...
currentWidget.update();break;default:
break;}}
},setValues: function(obj) {this._createFormElements();this._createRepeaters(obj,this);for(var i=0, len=this.formElements.length; i<len; i++) {var element = this.formElements[i];if (element.name == '') {continue};var namePath = new Array();namePath=element.name.split(".");var myObj=obj;var name=namePath[namePath.length-1];for(var j=1,len2=namePath.length;j<len2;++j) {var p=namePath[j - 1];var nameA=p.split("[");if (nameA.length > 1) {if(typeof(myObj[nameA[0]]) == "undefined") {myObj[nameA[0]]=[ ];}
nameIndex=parseInt(nameA[1]);if(typeof(myObj[nameA[0]][nameIndex]) == "undefined") {myObj[nameA[0]][nameIndex]={};}
myObj=myObj[nameA[0]][nameIndex];continue;}
if(typeof(myObj[p]) == "undefined") {myObj=undefined;break;};myObj=myObj[p];}
...
element.checked=false;if (typeof(myObj[name]) == 'undefined') continue;for (var j=0,len2=myObj[name].length; j<len2; ++j) {if(element.value == myObj[name][j]) {element.checked=true;}}
break;case "radio":
...
break;case "text":
var value="";if (typeof(myObj[name]) != 'undefined') {value = myObj[name];}
element.value=value;break;default:
break;}}
...
name=undefined;}
return obj;},isValid: function() {var stack=[];stack[0]=this;while(stack.length > 0) {var widget=stack.pop();for(var i=0;i<widget.children.length;++i) {stack.push(widget.children[i]);}
switch(widget.widgetType) {case "CurrencyTextbox":
case "DateTextbox":
case "IntegerTextbox":
case "InternetTextbox":
case "RegexpTextbox":
