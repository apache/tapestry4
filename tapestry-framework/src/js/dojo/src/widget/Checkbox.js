...
"dojo.widget.Checkbox",dojo.widget.HtmlWidget,{templatePath: dojo.uri.moduleUri("dojo", "widget/templates/Checkbox.html"),templateCssPath: dojo.uri.moduleUri("dojo", "widget/templates/Checkbox.css"),"class": "dojoCheckbox",_type: "checkbox",name: "",id: "",checked: false,tabIndex: "",value: "on",_groups: { },postMixInProperties: function(){dojo.widget.Checkbox.superclass.postMixInProperties.apply(this, arguments);if(!this.disabled && this.tabIndex==""){ this.tabIndex="0"; }},fillInTemplate: function(){this._setValue(this.checked);},postCreate: function(){var notcon = true;this.id = this.id !="" ? this.id : this.widgetId;if(this.id != ""){var labels = document.getElementsByTagName("label");if (labels != null && labels.length > 0){for(var i=0; i<labels.length; i++){if (labels[i].htmlFor == this.id){labels[i].id = (labels[i].htmlFor + "label");this._connectEvents(labels[i]);dojo.widget.wai.setAttr(this.domNode, "waiState", "labelledby", labels[i].id);break;}}
}}
...
"dojo.widget.a11y.Checkbox",dojo.widget.Checkbox,{templatePath: dojo.uri.moduleUri("dojo", "widget/templates/CheckboxA11y.html"),postCreate: function(args, frag){this.inputNode.checked=this.checked;if (this.disabled){this.inputNode.setAttribute("disabled",true);}
this._register();},_setValue: function( bool){this.checked = bool;this.inputNode.checked = bool;}}
);dojo.declare(
"dojo.widget.RadioButtonBase",null,{"class": "dojoRadioButton",_type: "radio",_onClick: function( e){if(!this.disabled && !this.checked){this.setValue(true);}
e.stopPropagation();this.onClick();},setValue: function( bool){this._setValue(bool);if(bool){dojo.lang.forEach(this._groups[this.name], function(widget){if(widget != this){widget._setValue(false);}}, this);}}
}
);dojo.widget.defineWidget(
