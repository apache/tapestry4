...
if(this.multiple){return a;} else {return a.shift();}},isSelected: function(obj){var data = this.store.get();for(var i=0; i<data.length; i++){if(data[i].src == obj){return true;}}
...
this.onFilter();},createSorter: function(info){var self=this;var sortFunctions=[];function createSortFunction(fieldIndex, dir){var meta=self.columns[fieldIndex];var field=meta.getField();return function(rowA, rowB){if(dojo.html.hasAttribute(rowA,"emptyRow")){ return 1; }
if(dojo.html.hasAttribute(rowB,"emptyRow")){ return -1; }
var a = self.store.getField(self.getDataByRow(rowA), field);var b = self.store.getField(self.getDataByRow(rowB), field);if(meta.sortFunction!=null){var ret = meta.sortFunction(a, b);} else {var ret = 0;if(a > b) ret = 1;if(a < b) ret = -1;}
return dir * ret;}}
...
var data = this.store.get();for(var i=0; i<data.length; i++){var row = this.createRow(data[i]);body.appendChild(row);idx++;}}
