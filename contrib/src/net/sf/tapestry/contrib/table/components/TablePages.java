...
public class TablePages extends BaseComponent {
    ...
    public int getCurrentPage() {
        return getTableView().getTableModel().getPagingState().getCurrentPage() + 1;
    }
    
    public void setCurrentPage(int nPage) {
        getTableView().getTableModel().getPagingState().setCurrentPage(nPage - 1);
    }
    ...
    public void changePage(IRequestCycle objCycle) {
        ...
        if (arrParamenters.length != 1 && !(arrParamenters[0] instanceof Integer)) {
            // error
            return;
        }
